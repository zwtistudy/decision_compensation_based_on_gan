...
class SAC_Base:
    ...
    def _random_action(self, d_action, c_action) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.d_action_sizes:
            ...
            random_d_action = torch.cat(random_d_actions, axis=-1).type(torch.float32)
    ...
    def _choose_action(self,
        ...
        if self.d_action_sizes:
            if self.discrete_dqn_like:
                ...
                d_action = torch.cat(d_action_list, dim=-1)
                ...
                if self.train_mode:
                    ...
                    d_dist_list = [distributions.OneHotCategorical(logits=torch.ones((batch, d_action_size),
                                                                                     device=self.device))
                                   for d_action_size in self.d_action_sizes]
                    random_d_action = torch.cat([dist.sample() for dist in d_dist_list], dim=-1)
    ...
    def get_m_data(self,
        ...
        m_indexes = torch.cat([bn_indexes, bn_indexes[:, -1:] + 1], dim=1)
        m_padding_masks = torch.cat([bn_padding_masks,
    ...
    def get_dqn_like_d_y(self,
        ...
        mask_stacked_q = torch.cat(mask_stacked_q_list, dim=-1)
        # [ensemble_q_sample, batch, d_action_summed_size]
